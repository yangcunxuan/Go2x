#!/usr/bin/env bash
# Relaunch only the two patrol_bridge processes inside a RUNNING perception
# container (keeps FAST-LIO alive and the session frame intact).
...
pkill -f 'patrol_bridge/bridge' || true
sleep 1
CLOUD_MAP_TOPIC=/__disabled ODOM_TOPIC=/Odometry \
  ros2 run patrol_bridge bridge >> /project/runtime/logs/patrol_bridge.log 2>&1 &
