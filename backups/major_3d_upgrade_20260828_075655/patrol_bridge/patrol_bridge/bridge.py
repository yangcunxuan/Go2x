...
class Bridge(Node):
    def __init__(self):
        ...
        self.pose={'x':0.0,'y':0.0,'yaw':0.0}; self.odom_seen=False; self.map_seen=False; self.cloud_map_seen=False; self.last_goal=None; self.nav_status='idle'; self.goal_handle=None
    ...
    def on_odom(self,msg):
        ...
        self.latest_odom_pose=(p.x,p.y,p.z,self.pose['yaw'])
        self.odom_seen=True
    ...
    def pose_msg(self,goal):
        p=PoseStamped();p.header.frame_id='map';p.header.stamp=self.get_clock().now().to_msg();p.pose.position.x=float(goal['x']);p.pose.position.y=float(goal['y']);yaw=float(goal.get('yaw',0));p.pose.orientation.z=math.sin(yaw/2);p.pose.orientation.w=math.cos(yaw/2);return p
