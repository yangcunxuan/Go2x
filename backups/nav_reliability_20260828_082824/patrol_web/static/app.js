...
renderPoints=()=>{renderPointsOriginal();for(const selector of ['#pointList','#navPointList']){$(selector)?.querySelectorAll('.list-item').forEach((row,index)=>{const p=checkpoints[index];if(p)row.querySelector('.item-meta').textContent=`X ${(+p.x).toFixed(2)} · Y ${(+p.y).toFixed(2)} · Z ${(+p.z||0).toFixed(2)} · ${Math.round((+p.yaw||0)*180/Math.PI)}°`})}};
...
function drawMaps(){cloudViewer?.render();navCloudViewer?.render();renderViewerMarkers(cloudViewer,'#cloudMarkers');renderViewerMarkers(navCloudViewer,'#navCloudMarkers');$('#mapInfo').textContent=cloudData.available?`累计 ${Number(cloudData.total_points||0).toLocaleString()} 点 · 显示 ${Number(cloudData.shown_points||0).toLocaleString()} · ${cloudData.frame||'map_level'}`:'等待 MID360 /Laser_map 三维点云'}
...
(async()=>{await refresh();showConfig();await loadData();$('#newTask').click();setInterval(refresh,2000);setInterval(async()=>{try{cloudData=await api('/api/cloud');cloudViewer?.update(cloudData);if(navCloudMode==='live')navCloudViewer?.update(cloudData)}catch(e){}drawMaps()},1500)})();
