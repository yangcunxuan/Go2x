...
class PointCloudViewer{
 constructor(canvas){
  this.canvas=canvas;this.yaw=-.75;this.pitch=.72;this.distance=12;this.radius=5;this.updatedAt=0;this.drag=null;
  ...
  canvas.addEventListener('pointerdown',e=>{this.drag={x:e.clientX,y:e.clientY};canvas.setPointerCapture?.(e.pointerId);canvas.classList.add('dragging')});
  canvas.addEventListener('pointermove',e=>{if(!this.drag)return;this.yaw+=(e.clientX-this.drag.x)*.007;this.pitch=Math.max(-1.45,Math.min(1.45,this.pitch+(e.clientY-this.drag.y)*.007));this.drag={x:e.clientX,y:e.clientY};this.render()});
  const stop=()=>{this.drag=null;canvas.classList.remove('dragging')};canvas.addEventListener('pointerup',stop);canvas.addEventListener('pointercancel',stop);
 ...
 update(data){
  ...
  this.updatedAt=data.updated_at;const source=data.points,b=data.bounds||[[0,0,0],[1,1,1]],center=[(b[0][0]+b[1][0])/2,(b[0][1]+b[1][1])/2,(b[0][2]+b[1][2])/2];
...
function drawMaps(){draw($('#navCanvas'));cloudViewer?.render();$('#mapInfo').textContent=cloudData.available?`累计 ${Number(cloudData.total_points||0).toLocaleString()} 点 · 显示 ${Number(cloudData.shown_points||0).toLocaleString()} · ${cloudData.frame||'map_level'}`:'等待 MID360 /Laser_map 三维点云'}
...
$('#saveMap').onclick=async()=>{const name=prompt('三维地图名称','patrol3d_'+new Date().toISOString().slice(0,10).replaceAll('-',''));if(!name)return;try{const j=await api('/api/maps/save',{name});toast(`PCD已保存：${j.name}（${Number(j.points||0).toLocaleString()}点）`)}catch(e){toast(e.message)}};
...
$('#runTask').onclick=async()=>{try{if(!currentTask)throw Error('请先保存任务');await api('/api/tasks/run',{id:currentTask.id});toast('任务已启动')}catch(e){toast(e.message)}};$('#cancelTask').onclick=async()=>{await api('/api/tasks/stop',{});toast('已请求停止任务')};
