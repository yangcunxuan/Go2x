...
let statusData={},checkpoints=[],tasks=[],c12Presets=[],mapData={available:false},cloudData={available:false},maps=[],workspace,currentTask=null,view={scale:24,ox:0,oy:0},cloudViewer,navCloudViewer,navCloudMode='live';
...
async function refresh(){try{statusData=await api('/api/status');const r=statusData.robot,g=statusData.go2||{},c=statusData.c12,p=r.pose||{};$('#robotDot').className='dot '+((r.online||g.online)?'ok':'warn');$('#robotStatus').textContent=g.online?'GO2 状态在线':(r.online?'定位在线':'机器狗等待接入');$('#cameraDot').className='dot '+(c.visible&&c.thermal?'ok':'warn');$('#cameraStatus').textContent=c.visible&&c.thermal?'C12 双光在线':'C12 等待连接';$('#metricPose').textContent=`${(+p.x||0).toFixed(2)}, ${(+p.y||0).toFixed(2)}, ${(+p.z||0).toFixed(2)}`;$('#metricYaw').textContent=`${Math.round((+p.yaw||0)*180/Math.PI)}°`;$('#metricBattery').textContent=g.battery_soc==null?'--%':`${g.battery_soc}% · ${(+g.voltage_v||0).toFixed(1)}V`;const imu=g.imu_rpy||[];$('#metricAttitude').textContent=imu.length?`${(imu[0]*180/Math.PI).toFixed(1)}° / ${(imu[1]*180/Math.PI).toFixed(1)}°`:'--';$('#metricBody').textContent=g.body_height==null?'--':`${(+g.body_height).toFixed(2)}m · M${g.mode}`;$('#metricTemp').textContent=g.imu_temperature_c==null?'--':`${(+g.imu_temperature_c).toFixed(0)}°C / ${(+g.max_motor_temperature_c||0).toFixed(0)}°C`;const run=statusData.task_run;$('#runState').className='run-state '+run.state;$('#runState').textContent=run.state==='idle'?'暂无运行任务':({running:'任务执行中',completed:'任务已完成',failed:'任务失败',canceled:'任务已停止'}[run.state]||run.state);$('#runMessage').textContent=run.message||'在“积木任务”中创建并运行巡逻流程。';const task=tasks.find(t=>t.id===run.task_id),total=task?.steps?.length||0;$('#runProgress').style.width=total?`${Math.min(100,(run.step/total)*100)}%`:'0';const navRunning=!!statusData.services.navigation?.running;$('#navState').textContent=navRunning?`状态：${r.nav_status||'启动中'} · XYZ ${(+p.x||0).toFixed(2)}, ${(+p.y||0).toFixed(2)}, ${(+p.z||0).toFixed(2)}`:'导航未启动';$('#startNav').textContent=navRunning?'导航运行中':'启动导航';$('#startMapping').textContent=statusData.services.mapping?.running?'建图运行中':'开始三维建图'}catch(e){$('#robotStatus').textContent='巡逻服务离线'}}
async function loadData(){checkpoints=await api('/api/checkpoints');tasks=await api('/api/tasks');try{c12Presets=await api('/api/c12/presets')}catch(e){c12Presets=[]}try{maps=(await api('/api/maps')).maps||[]}catch(e){maps=[]}$('#metricPoints').textContent=checkpoints.length;$('#metricTasks').textContent=tasks.length;renderPoints();renderTasks();renderMaps();try{cloudData=await api('/api/cloud')}catch(e){}cloudViewer?.update(cloudData);if(navCloudMode==='live')navCloudViewer?.update(cloudData);drawMaps();if(workspace)workspace.updateToolbox(toolbox())}
...
const renderPointsOriginal=renderPoints;
renderPoints=()=>{renderPointsOriginal();for(const selector of ['#pointList','#navPointList']){$(selector)?.querySelectorAll('.list-item').forEach((row,index)=>{const p=checkpoints[index];if(p){const map=p.map_name&&p.map_name!=='__live__'?` · 地图 ${p.map_name}`:(p.map_name==='__live__'?' · 本次实时地图':' · 未绑定地图');row.querySelector('.item-meta').textContent=`X ${(+p.x).toFixed(2)} · Y ${(+p.y).toFixed(2)} · Z ${(+p.z||0).toFixed(2)} · ${Math.round((+p.yaw||0)*180/Math.PI)}°${map}`}})}};

function renderMaps(){const box=$('#mapList');if(!box)return;box.replaceChildren();if(!maps.length){box.innerHTML='<div class="list-empty">暂无已保存三维地图</div>';return}maps.forEach(m=>{const row=document.createElement('div');row.className='list-item'+(m.active?' active-map':'');row.innerHTML='<div class="item-head"><span class="item-name"></span><span class="map-active"></span></div><div class="item-meta"></div><div class="item-actions"></div>';row.querySelector('.item-name').textContent=m.name;row.querySelector('.map-active').textContent=m.active?'使用中':'';row.querySelector('.item-meta').textContent=`${Number(m.points||0).toLocaleString()}点 · ${(m.bytes/1048576).toFixed(1)}MB · ${m.nav_ready?'导航层就绪':'无导航层'}`;const viewButton=document.createElement('button');viewButton.textContent='查看';viewButton.onclick=async()=>{try{const data=await api('/api/maps/cloud?name='+encodeURIComponent(m.name));navCloudMode=m.name;navCloudViewer.update(data);$('#navMapTitle').textContent='三维地图：'+m.name;drawMaps()}catch(e){toast(e.message)}};const use=document.createElement('button');use.className='primary';use.textContent=m.active?'已选用':'选用';use.disabled=m.active||!m.nav_ready;use.onclick=async()=>{try{await api('/api/maps/activate',{name:m.name});await loadData();toast('已选用地图：'+m.name)}catch(e){toast(e.message)}};const cont=document.createElement('button');cont.textContent='续建';cont.onclick=async()=>{try{const j=await api('/api/maps/continue',{name:m.name});toast(j.message)}catch(e){toast(e.message)}};row.querySelector('.item-actions').append(viewButton,use,cont);box.append(row)})}
function applyFreshnessUI(){const r=statusData.robot||{},mapping=!!statusData.services?.mapping?.running;if(!r.online){$('#metricPose').textContent='定位离线';$('#metricYaw').textContent='--';if(!statusData.services?.navigation?.running)$('#navState').textContent=mapping?'等待FAST-LIO实时定位':'导航不可用 · 请先开始本次三维建图'}$('#startNav').disabled=!mapping||!r.online||!r.cloud_map_online;$('#startNav').title=$('#startNav').disabled?'导航必须保持本次FAST-LIO建图和实时点云在线':''}
...
class PointCloudViewer{
 ...
 groundZAt(x,y){if(!this.source?.length)return 0;let best=[];for(let i=0;i<this.source.length;i+=3){const dx=this.source[i]-x,dy=this.source[i+1]-y,d=dx*dx+dy*dy;if(d<1.0)best.push([d,this.source[i+2]])}best.sort((a,b)=>a[0]-b[0]);const z=best.slice(0,20).map(v=>v[1]).sort((a,b)=>a-b);return z.length?z[Math.floor(z.length*.25)]:0}
 ...
 update(data){
  ...
  this.updatedAt=data.updated_at;const source=data.points;this.source=source;const b=data.bounds||[[0,0,0],[1,1,1]],center=[(b[0][0]+b[1][0])/2,(b[0][1]+b[1][1])/2,(b[0][2]+b[1][2])/2];this.center=center;
...
cloudViewer=new PointCloudViewer($('#cloudCanvas'));navCloudViewer=new PointCloudViewer($('#navCloudCanvas'));
function renderViewerMarkers(viewer,selector){const layer=$(selector);if(!layer)return;layer.replaceChildren();checkpoints.forEach(point=>{const screen=viewer?.project(point.x,point.y,+point.z||0);if(!screen)return;const marker=document.createElement('span');marker.className='cloud-marker';marker.style.left=screen.x+'px';marker.style.top=screen.y+'px';marker.textContent=point.name;layer.append(marker)});const p=statusData.robot?.pose;if(!p||!statusData.robot?.online)return;const here=viewer?.project(+p.x||0,+p.y||0,+p.z||0),ahead=viewer?.project((+p.x||0)+Math.cos(+p.yaw||0), (+p.y||0)+Math.sin(+p.yaw||0),+p.z||0);if(!here)return;const robot=document.createElement('span');robot.className='cloud-robot';robot.style.left=here.x+'px';robot.style.top=here.y+'px';if(ahead)robot.style.setProperty('--heading',Math.atan2(ahead.y-here.y,ahead.x-here.x)+'rad');robot.innerHTML=`<i></i><b>GO2<br>X ${(+p.x||0).toFixed(2)} Y ${(+p.y||0).toFixed(2)} Z ${(+p.z||0).toFixed(2)}</b>`;layer.append(robot)}
function drawMaps(){cloudViewer?.render();navCloudViewer?.render();renderViewerMarkers(cloudViewer,'#cloudMarkers');renderViewerMarkers(navCloudViewer,'#navCloudMarkers');const age=cloudData.updated_at?Date.now()/1000-cloudData.updated_at:Infinity,fresh=age<4;$('#mapInfo').textContent=cloudData.available?(fresh?`实时 · 累计 ${Number(cloudData.total_points||0).toLocaleString()} 点 · 显示 ${Number(cloudData.shown_points||0).toLocaleString()} · ${cloudData.frame||'map_level'}`:`点云已中断 ${Math.round(age)} 秒 · 当前画面是缓存，禁止导航`):'等待 MID360 /Laser_map 三维点云';$('#mapInfo').className=fresh?'muted':'stale-warning'}
addEventListener('resize',drawMaps);setInterval(drawMaps,1000);
function openPoint(x,y,yaw,z=null){if(z==null)z=cloudViewer?.groundZAt(x,y)||0;$('#pointName').value='';$('#pointX').value=x.toFixed(2);$('#pointY').value=y.toFixed(2);$('#pointZ').value=(+z||0).toFixed(2);$('#pointYaw').value=Math.round(yaw*180/Math.PI);$('#pointDialog').showModal()}
$('#addCurrent').onclick=()=>{const p=statusData.robot?.pose||{x:0,y:0,z:0,yaw:0};openPoint(+p.x||0,+p.y||0,+p.yaw||0,+p.z||0)};
$('#pickPoint').onclick=()=>cloudViewer?.beginPick();
$('#confirmPoint').onclick=async e=>{e.preventDefault();try{await api('/api/checkpoints/save',{name:$('#pointName').value,x:+$('#pointX').value,y:+$('#pointY').value,z:+$('#pointZ').value,yaw:+$('#pointYaw').value*Math.PI/180});$('#pointDialog').close();await loadData();toast('三维巡查点已保存')}catch(err){toast(err.message)}};

$('#startMapping').onclick=async()=>{try{await api('/api/mapping/start',{});toast('MID360三维建图正在启动')}catch(e){toast(e.message)}};$('#stopMapping').onclick=async()=>{try{toast('正在自动保存当前地图…');const j=await api('/api/mapping/stop',{});toast(j.autosave?`地图已自动保存：${j.autosave.name}`:(j.save_error?'建图已停止，自动保存失败：'+j.save_error:'建图服务已停止'))}catch(e){toast(e.message)}};$('#startNav').onclick=async()=>{try{toast('正在核对实时定位并准备当前地图…');await api('/api/navigation/start',{});toast('导航服务已就绪，可选择本次地图的巡查点')}catch(e){toast(e.message)}};$('#stopNav').onclick=async()=>{await api('/api/navigation/stop',{});toast('导航服务已停止')};
$('#stopRadar').onclick=async()=>{if(!confirm('确认停止MID360采集、FAST-LIO和导航进程？硬件电源不会被切断。'))return;try{const j=await api('/api/radar/stop',{});toast(j.message)}catch(e){toast(e.message)}};
$('#liveCloud').onclick=()=>{navCloudMode='live';navCloudViewer.update(cloudData);$('#navMapTitle').textContent='实时三维地图';drawMaps()};
$$('.robot-action').forEach(button=>button.onclick=async()=>{const label=button.textContent;if(!confirm(`确认让机器狗执行“${label}”？请确保四周及关节附近无人，并握住遥控器。`))return;try{await api('/api/robot/action',{action:button.dataset.action,confirm:true});toast('已下发：'+label)}catch(e){toast(e.message)}});
...
(async()=>{await refresh();applyFreshnessUI();showConfig();await loadData();$('#newTask').click();setInterval(async()=>{await refresh();applyFreshnessUI()},2000);setInterval(async()=>{try{cloudData=await api('/api/cloud');cloudViewer?.update(cloudData);if(navCloudMode==='live')navCloudViewer?.update(cloudData)}catch(e){}drawMaps()},1500)})();
