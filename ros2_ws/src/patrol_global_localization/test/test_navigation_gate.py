...
from patrol_global_localization.navigation_gate import (  # noqa: E402
    angle_diff, cluster_hypotheses, convert_goal, navigation_gate, quat_matrix,
...
def test_gicp_known_transform_recovery():
    ...
    try:
        ...
        pytest.skip('rclpy unavailable on this host')
    rng = np.random.default_rng(9)
    target = np.column_stack([
        rng.uniform(-10, 10, 20000), rng.uniform(-10, 10, 20000),
        rng.uniform(0, 3, 20000)])
    # source points are generated by TRANSFORMING target points with
    # T_true (target -> source); aligning source back onto target therefore
    # recovers inverse(T_true).
    true_t = yaw_matrix(1.2, -0.8, 0.05, math.radians(12))
    source = target @ true_t[:3, :3].T + true_t[:3, 3]
    source = source + rng.normal(0, 0.01, source.shape)
    expected = np.linalg.inv(true_t)
    reg = Registration.__new__(Registration)
    # perturbed initial guess: 0.5 m and 5 deg off
    init = expected @ yaw_matrix(0.5, 0.3, 0.0, math.radians(-5))
    result = reg.align(target, source, init, 0.5, 2.5)
    pos_err = np.linalg.norm(result['T'][:3, 3] - expected[:3, 3])
    yaw_err = angle_diff(yaw_of(result['T']), yaw_of(expected))
    assert pos_err < 0.15, pos_err
    assert yaw_err < math.radians(3), yaw_err
    assert result['quality_ok'], result


# ---------- quaternion matrix signs (P0 audit: sign errors) ----------
def test_quat_matrix_rotation_roundtrip():
    """Lock the quaternion -> rotation matrix signs used across the stack."""
    for yaw in (0.0, 0.3, -1.2, math.pi):
        q = (0.0, 0.0, math.sin(yaw / 2), math.cos(yaw / 2))
        r = quat_matrix(*q)
        assert abs(math.atan2(r[1, 0], r[0, 0]) - yaw) < 1e-9
        assert np.abs(r.T @ r - np.eye(3)).max() < 1e-9
        assert abs(np.trace(r) - (1 + 2 * math.cos(yaw))) < 1e-9
